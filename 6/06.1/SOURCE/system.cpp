...
double const vmax = 5.0; // inizializzo velocita' massima Ex.4///////////////////////////////////////////////////////////////

double const tmin = 0.5; // inizializzo temperatura minima Ex.6/////////////////////////////////////////////////////////
double const tmax = 2.0; // inizializzo temperatura massima Ex.6/////////////////////////////////////////////////////////

void System ::step()
{ // Perform a simulation step //Siamo nella dinamica (nei cicli dei blocchi)
    if (_sim_type == 0)
        this->Verlet(); // Perform a MD step
    // Inserire qui il for sulla temperatura
    else
        for (int i = 0; i < _npart; i++)
            this->move(int(_rnd.Rannyu() * _npart)); // Perform a MC step on a randomly choosen particle
    _nattempts += _npart;                            // update number of attempts performed on the system //Muovo tutte le particelle ad ogni step
    return;
}

// Calcolo la forza su ogni particella e implemento l'algoritomo di Verlet
void System ::Verlet()
{
...
double System ::Force(int i, int dim)
{
    ...
    for (int j = 0; j < _npart; j++)
    {
        ...
        { // se non è la stessa particella
            ...
            dr = sqrt(dot(distance, distance)); // dot è funzione di prodotto scalare
            if (dr < _r_cut)
            {                                                                  // Controllo sul cutoff del potenziale
                f += distance(dim) * (48.0 / pow(dr, 14) - 24.0 / pow(dr, 8)); // Forza ricavata dal potenziale di Lennard-Jones
            }
        }
    }
    return f;
...
{ // Propose a MC move for particle i
    ...
    { // Gibbs sampler for Ising
        ...
        // non devo moltiplicare per  _particle(i).getspin() perchè lo spin della particella i non influisce sul campo
        // double delta_E = 2.0 *  _particle(i).getspin() *
        //              ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );
        double acceptance = 1.0 / (1.0 + exp(-_beta * delta_E));
        // Come mostra la riga sopra, nel modello di Ising, la temperatura influenza la probabilità
        // di accettazione delle configurazioni (tramite il parametro (beta = 1 / T)),
        // modificando la configurazione degli spin e quindi l'energia potenziale.
...
void System ::initialize_velocities()
{
    ...
    if (_restart)
    {
        ...
        if (cinf.is_open())
        {
            ...
            if (ncoord != _npart)
            {
                // cout << "Number of coordinates in conf-1.xyz: " << ncoord << endl;
                // cout << "Number of particles in input.dat: " << _npart << endl;
    ...
    { // TO BE TEMPORARILY CHANGED IN EXERCISE 4
        ...
        // Calcolo la velocità di deriva (del cm) da ogni direzione
        for (int idim = 0; idim < _ndim; idim++)
            sumv(idim) = sumv(idim) / double(_npart);
        double sumv2 = 0.0, scalef;
        // da ogni particella tolgo la velocità di deriva (del cm) da ogni direzione
        for (int i = 0; i < _npart; i++)
        {
            vx(i) = vx(i) - sumv(0);
            vy(i) = vy(i) - sumv(1);
        ...
        // Sottraendo la velocità di deriva ho reso le velocità imcompatibili con la T desiderata
...
{ // Initialize data members used for measurement of properties
    ...
    // Leggo file properties.dat che contiene i nomi delle variabili (proprieta' del sistema) da calcolare
    ...
    if (input.is_open())
    {
        while (!input.eof())
        {
            ...
            {
                ofstream coutp("../OUTPUT/potential_energy.dat"); // Preparo il file di output per l'energia potenziale
                coutp << "#     BLOCK:  ACTUAL_PE:     PE_AVE:      ERROR:" << endl;
                coutp.close();
                _nprop++; // Incremento il contatore delle proprieta' da calcolare per passare alla successiva
                _index_penergy = index_property;
                _measure_penergy = true; // Il booleano dell'energia potenziale diventa true -> da calcolare
                index_property++;        // Passo alla successiva
            ...
            else if (property == "TOTAL_ENERGY")
            {
                ...
                coutt << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:      TEMPERATURE:" << endl;
                coutt.close();
                _nprop++;
            ...
            else if (property == "POFV")
            {
                ...
                ofstream coutpv("../OUTPUT/pofv.dat"); // Nel file di output pofv.dat scriveremo p of v incolonnato bene
                // coutpv << "# VELOCITY:     AVE_POFV:        ERROR:" << endl;
                coutpv << "#     BLOCK:       POFV:   AVE_POFV:      ERROR:" << endl;
                coutpv.close();
                input >> _n_bins_v; // Leggo il numero di bins per la distribuzione delle velocita'
                _nprop += _n_bins_v;
            ...
            else if (property == "MAGNETIZATION")
            {
                ...
                coutpr << "#     BLOCK:   ACTUAL_M:     M_AVE:       ERROR:      TEMPERATURE:" << endl;
                coutpr.close();
                _nprop++;
            ...
            else if (property == "SPECIFIC_HEAT")
            {
                ...
                coutpr << "#     BLOCK:   ACTUAL_CV:    CV_AVE:      ERROR:      TEMPERATURE:" << endl;
                coutpr.close();
                _nprop++;
                _measure_cv = true;
            ...
            else if (property == "SUSCEPTIBILITY")
            {
                ...
                coutpr << "#     BLOCK:   ACTUAL_X:     X_AVE:       ERROR:      TEMPERATURE:" << endl;
                coutpr.close();
                _nprop++;
                _measure_chi = true;
...
{ // Measure properties
    ...
    vec distance;
    distance.resize(_ndim);
    double pofv_temp = 0.0;        // temporary accumulator for distribution of velocities
    ...
    if (_measure_penergy or _measure_pressure or _measure_gofr)
    {
        for (int i = 0; i < _npart - 1; i++)
        {
            ...
            {                                                                                                      // Due for su tutte le coppie possibili di particelle
                distance(0) = this->pbc(_particle(i).getposition(0, true) - _particle(j).getposition(0, true), 0); // this per chiamare un'altra funzione di System
                distance(1) = this->pbc(_particle(i).getposition(1, true) - _particle(j).getposition(1, true), 1);
                distance(2) = this->pbc(_particle(i).getposition(2, true) - _particle(j).getposition(2, true), 2);
    ...
    // POFV ///Calcolo la distribuzione di velocita' delle particelle
    if (_measure_pofv)
    {
        int bin;
        for (int i = 0; i < _npart; i++)
    ...
    if (_measure_tenergy)
    {
        ...
        { // energia interana per modello di Ising 1D
            double s_i, s_j;
            for (int i = 0; i < _npart; i++)
            {
    ...
    if (_measure_cv) {
        if (_sim_type >= 2) {
            ...
            _measurement(_index_cv) = (tenergy_temp2 - pow(tenergy_temp, 2)) * _beta * _beta; // Non moltiplico per K-b perchè uguale ad 1
            */
    ...
    if (_measure_chi) {
        if (_sim_type >= 2) {
            double s_i;
            for (int i = 0; i < _npart; i++){
                chi_temp += double(_particle(i).getspin());
            }
            chi_temp = pow((chi_temp), 2) / double(_npart);

            _measurement(_index_chi) = chi_temp * _beta;
        }
    }
...
// Fa le medie delle proprieta' accumulate in _block_av per ogni blocco
// e scrive i risultati nei file di output
void System ::averages(int blk) {

    ofstream coutf;
    double average, sum_average, sum_ave2;

    _average = _block_av / double(_nsteps); // Vettore che contiene le medie
    _global_av += _average;
    _global_av2 += _average % _average; // % -> element-wise multiplication (prodotto componente per componente di due vettori lunghi uguali)
    ...
    if (_measure_penergy) {
        ...
        coutf << setw(12) << blk
              << setw(12) << average
              << setw(12) << sum_average / double(blk) // valore medio progressivo
    ...
    if (_measure_magnet) {
        ...
        coutf << setw(12) << blk
              ...
              << setw(12) << sum_average / double(blk)
              << setw(12) << this->error(sum_average, sum_ave2, blk)
              << setw(12) << _temp << endl;       //stampo la temperatura
        coutf.close();
    }
    ...
    // TO BE FIXED IN EXERCISE 6
    if (_measure_cv) {
        //devo concludere qui il calcolo del cv perchè contiene già la media
        _global_av(_index_cv) -= _average(_index_cv); //tolgo H2 da _global_av che contiene la media calcolata prima
        _global_av2(_index_cv) -= _average(_index_cv) * _average(_index_cv);
        average = _beta*_beta * (_average(_index_cv)- pow(_average(_index_tenergy),2))*_npart; 
        _global_av(_index_cv) += average;//calcolo la media progressiva con la grandezza interessata
