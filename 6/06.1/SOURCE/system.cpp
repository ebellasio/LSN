...
void System :: step(){ // Perform a simulation step //Siamo nella dinamica (nei cicli dei blocchi)
...
double System :: Force(int i, int dim){
  ...
  for (int j=0; j<_npart; j++){
    if(i != j){     //se non è la stessa particella
      ...
      if(dr < _r_cut){      //Controllo sul cutoff del potenziale
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      if(i%6 == 0) vx(i) = v_t;
      else if(i%6 == 1) vx(i) = -v_t;
      else if(i%6 == 2) vy(i) = v_t;
      else if(i%6 == 3) vy(i) = -v_t;
      else if(i%6 == 4) vz(i) = v_t;
      else if(i%6 == 5) vz(i) = -v_t;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){           // Due for su tutte le coppie possibili di particelle
  ...
  if (_measure_pofv){
    ...
    for (int i=0; i<_n_bins_v; i++) _measurement(_index_pofv + i) /= double(_npart); //* _bin_size_v);
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
...
// TO BE FIXED IN EXERCISE 6 
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_cv){
    //double sum = 0.0;          //Alternativa
    if (_sim_type >= 2){ 
      double s_i, s_j;
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        //sum +=  - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
        tenergy_temp += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
        tenergy_temp2 += pow( - _J * s_i * s_j - 0.5 * _H * (s_i + s_j) ,2);
      }
      //tenergy_temp = sum / double(_npart);
      //tenergy_temp2 = pow(sum, 2)/ double(_npart);
      tenergy_temp /= double(_npart);  
      tenergy_temp2 /= double(_npart);
      _measurement(_index_cv) = (tenergy_temp2 - pow(tenergy_temp,2)) * _beta*_beta;      //Non moltiplico per K-beta perchè uguale ad 1
    }
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if (_measure_chi){
    // double sum = 0.0;          //Alternativa incompleta
    if (_sim_type >= 2){ 
      double s_i;
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
