...
mat distanze_citta ( int n, char schema, Random &rnd, int rank){
    ...
    // Q significa che le città sono distribuite in un quadrato di lato tra -1 e 1
        ...
        for ( int i = 0; i < n; i++ ){
            in >> coordinate(i, 0) >> coordinate(i, 1); //leggo le coordinate dal file
        }
        in.close();
    } else{
        cerr << "Errore: schema non valido" << endl;
    }
...
//Funzione che gestisce la migrazione tra i processi
void migrazione (Popolazione &pop, int rank, int size, mat D, Random &rnd) {
    int rank1 = 0, rank2 = 0; //processi che scambiano gli individui
    if (rank == 0) {
        rank1 = rnd.Rannyu(0, size); //primo processo scelto casualmente
        do {
            rank2 = rnd.Rannyu(0, size); //secondo processo scelto casualmente
        } while (rank2 == rank1); //assicurarsi che siano diversi
    }

    MPI_Bcast(&rank1, 1, MPI_INT, 0, MPI_COMM_WORLD); //comunico a tutti i processi il primo processo scelto
    MPI_Bcast(&rank2, 1, MPI_INT, 0, MPI_COMM_WORLD); //comunico a tutti i processi il secondo processo scelto

    if (rank == rank1 || rank == rank2){
        Cromosoma best = pop.GetBest(); //prendo il migliore individuo del processo
        vector<int> send_info = best.Serialize(); //serializzo il cromosoma in un vettore di int
        vector<int> recv_info(send_info.size()); //vettore per ricevere il cromosoma

        int pair_rank = (rank == rank1) ? rank2 : rank1; //processo con cui scambiare l'individuo

        MPI_Sendrecv(send_info.data(), send_info.size(), MPI_INT, pair_rank, 0, //invio il cromosoma al processo scelto
                     recv_info.data(), recv_info.size(), MPI_INT, pair_rank, 0, //ricevo il cromosoma dal processo scelto
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE); //scambio i cromosomi tra i due processi

        Cromosoma received(pop.GetN()); //creo un nuovo cromosoma con lo stesso numero di geni
        received.Deserialize(recv_info); //deserializzo il cromosoma ricevuto
        received.Set_distances(D); //setto la matrice delle distanze
        received.fitness(); //calcolo la fitness del cromosoma ricevuto

        pop.SetBest(received); //sostituisco il migliore individuo con quello ricevuto
        pop.Sort(); //ordino la popolazione
    }
}
