...
const double vmax = 5.0; // inizializzo velocita' massima Ex.4///////////////////////////////////////////////////////////////
    ...
    if (input.is_open()) {
        while (!input.eof()) {
            ...
            if (property == "POTENTIAL_ENERGY") {
                ...
                index_property++;
                // TO BE FIXED IN EXERCISE 7
                _ptail = (32.0 * M_PI * _rho) / (9.0 * pow(_r_cut, 9)) - (32.0 * M_PI * _rho) / (6.0 * pow(_r_cut, 3)); 
            }
            else if (property == "GOFR") {
                ofstream coutgr("../OUTPUT/gofr.dat");
                //coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
                coutgr << "#     BLOCK:     DISTANCE:     GOFR:     AVE_GOFR:     ERROR:" << endl;
                coutgr.close();
                input >> _n_bins;
                _nprop += _n_bins;
...
void System ::measure(){ // Measure properties
    ...
    vec distance;
    distance.resize(_ndim);
    double pofv_temp = 0.0;        // temporary variable for POFV
    ...
    if (_measure_penergy or _measure_pressure or _measure_gofr) {
        for (int i = 0; i < _npart - 1; i++) {
            for (int j = i + 1; j < _npart; j++) {
                ...
                if (_measure_gofr){
                    bin = int(dr / _bin_size);   //indice del bin
                    if (bin < _n_bins)           //se il bin esiste
                        _measurement(_index_gofr + bin) += 2.0; //aggiorno (2.0 per la simmetria) il contatore del bin
                }
                if (dr < _r_cut){
                    if (_measure_penergy)
    ...
    // TEMPERATURE ///////////////////////////////////////////////////////////////
    if (_measure_temp and _measure_kenergy and _sim_type != 1)
        _measurement(_index_temp) = (2.0 / 3.0) * kenergy_temp;
    // PRESSURE //////////////////////////////////////////////////////////////////
    if (_measure_pressure){
        //Con MC uso la temperatura al posto dell'energia cinetica
        if(_sim_type == 1)        
