...
// Funzione che calcola (-hbar/2m * derivata seconda di psi)/psi
double hpsi (double x, double mu, double sigma){
  double num = 1./(2*sigma*sigma)*(exp(-pow(x-mu,2)/(sigma*sigma))*(-1+pow((x-mu),2)/(sigma*sigma)) + 
          exp(-pow(x+mu,2)/(sigma*sigma))*(-1+pow((x+mu),2)/(sigma*sigma)));
  double den = exp(-pow(x-mu,2)/(2*sigma*sigma)) + exp(-pow(x+mu,2)/(2*sigma*sigma));
  return num / den;
}

double H (double xold){      //Sistemo struttura codice !!!!!!!!!!!!!!!!!!
  double xnew, H;
  for (int i=0; i<M; i++){
    H = 0;
    for (int j=0; j<N; j++){
      xnew = xold  + delta * (2*rnd.Rannyu() - 1);
      if (metro){
        H += hpsi (xnew, mu, sigma);
        xold = xnew;
      } else{
      H += hpsi (xold, mu, sigma);
    }
    }
  }
...
//Algortimo di Metropolis modificato per l'hamiltoniana della funzione d'onda degli stati quantistici
bool System :: metro(double xnew, double xold, double mu, double sigma){ // Metropolis algorithm
  bool decision = false;
  double acceptance = pow(exp(-pow(xnew-mu,2)/(2*sigma*sigma)) + exp(-pow(xnew+mu,2)/(2*sigma*sigma)),2) /
                    (pow(exp(-pow(xold-mu,2)/(2*sigma*sigma)) + exp(-pow(xold+mu,2)/(2*sigma*sigma)),2));
