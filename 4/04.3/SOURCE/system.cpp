...
void System ::step(){ // Perform a simulation step //Siamo già nella dinamica (nei cicli dei blocchi)
    if (_sim_type == 0)
...
void System ::initialize_velocities() {
    ...
    if (_restart){ //Leggo da file conf-1.xyz le posizioni iniziali
        ifstream cinf;
...
void System ::initialize_properties(){ // Initialize data members used for measurement of properties
    ...
    if (input.is_open()){
        while (!input.eof()){
            ...
            if (property == "POTENTIAL_ENERGY"){
                ...
                //stampo a video il numero di bins e la loro ampiezza
                cout << "n_bins_v= " << _n_bins_v << "  bin_size_v= " << _bin_size_v << endl;
            }
    ...
    cout << "List of properties to be measured:" 
            ...
            << (_measure_tenergy ? " TOTAL_ENERGY" : "")
            << (_measure_temp ? " TEMPERATURE" : "")
            << (_measure_pressure ? " PRESSURE" : "")
            << (_measure_gofr ? " GOFR" : "")
            << (_measure_pofv ? " POFV" : "")
            << (_measure_magnet ? " MAGNETIZATION" : "")
            << (_measure_cv ? " SPECIFIC_HEAT" : "")
            << (_measure_chi ? " SUSCEPTIBILITY" : "")
            << endl;
