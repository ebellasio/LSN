...
double const vmax = 5.0; //inizializzo la velocita' massima per POFV Ex.4///////////////////////////////////////////////////////////////
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    cinf.close();
  } else { //qua modifico per mettere una distribuzione iniziale diversa, come una delta/////////////////////////////////////////////////////////////////////////////////
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();

    double const v_t = 1.05; //velocita' probabile per le condizioni presenti///////////////////////////////////////////////////
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        cout << "NBINS: " << _n_bins_v << "\tBINSIZE: " << _bin_size_v << endl;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x/2.0, 0)); //divido per 2 per configurazione a minima entropia////////////////////////////////////////////////////////////////////////////////////
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pofv){
    ...
    for(int i=0; i<_npart; i++) {
      ...
      index = min(int(floor(pofv_temp / vmax * double(_n_bins_v))), _n_bins_v-1); //controllo che sia effettivamente un indice
      _measurement(_index_pofv + index) += 1.0;
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
...
void System :: averages(int blk){
  ...
  // POFV ////////////////////////////////////////////////////////////////////////////////////TO BE FIXED IN EXERCISE 4///////////////////vettore di proprieta'
  if (_measure_pofv){
    coutf.open("../OUTPUT/pofv.dat",ios::app);
    for(int i=0; i<_n_bins_v; i++){ //faccio un ciclo su tutti i bin
      average  = _average(_index_pofv+i); //prendo il bin relativo
    ...
    if(blk == _nblocks){ //alla fine della simulazione scrivo il pofv finale per l'istogramma
      coutf.open("../OUTPUT/pofv_final.dat",ios::app);
      for(int i=0; i<_n_bins_v; i++){ //faccio un ciclo su tutti i bin
        average  = _average(_index_pofv+i); //prendo il bin relativo
        sum_average = _global_av(_index_pofv+i);
        sum_ave2 = _global_av2(_index_pofv+i);
    ...
    coutf.open("../OUTPUT/pofv_max.dat",ios::app); //scrivo anche un file che mostra la convergenza per il bin in corrispondenza della velocita' massima
    int i_max = 8;
    average  = _average(_index_pofv+i_max); //prendo solo il bin massimo
    sum_average = _global_av(_index_pofv+i_max);
    sum_ave2 = _global_av2(_index_pofv+i_max);
    coutf << setw(12) << blk
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
